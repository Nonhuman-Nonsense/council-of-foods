...
interface SummaryProps {
    ...
    meetingId?: string | number; // Council.tsx passes currentMeetingId which can be string|number|null
}
 ...
 */
function Summary({ summary, meetingId }: SummaryProps) {
    const isMobile = useMobile();
    const pdfElementRef = useRef<{ createPdf: () => void }>(null);
    ...
    const summaryWrapper: React.CSSProperties = {
        ...
        WebkitMask: "linear-gradient(to bottom, rgb(0, 0, 0) 0, rgb(0,0,0) 93%, rgba(0,0,0, 0) 100% ) repeat-x", // Add webkit prefix just in case
    };
            ...
            <div style={wrapper}>
                ...
                >
                    ...
                    <div style={{ display: "flex", flexDirection: "row", margin: "20px 0", justifyContent: "space-between" }}>
                        <div>
                            <img style={{ width: isMobile ? '80px' : '110px', paddingRight: "10px" }} src='/logos/council_logo_white.svg' alt="council of foods logo" />
                        </div>
                    ...
                    <div
                        ...
                        style={protocolStyle}
                    >
                        {/* @ts-ignore: marked can return Promise if async but usually sync string? and parse expects string */}
                        {summary.text && parse(marked(summary.text) as string)}
                        <hr /><br />
                        <Disclaimer />
                    </div>
                </div>
 ...
 */
const PDFToPrint = forwardRef<{ createPdf: () => void }, PDFToPrintProps>((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        createPdf() {
            // @ts-ignore: Assuming Tinos.ts has side effects for jsPDF fonts or we just import it
            import("../../Tinos").then(() => {
                const pdf = new jsPDF("p", "pt", "a4");
                // @ts-ignore: setFont might not know Tinos if types aren't updated, but it works at runtime
                pdf.setFont("Tinos");
                if (protocolRef.current) {
                    pdf.html(protocolRef.current, {
                        ...
                        autoPaging: 'text',
                        margin: [50, 50, 50, 50],
                        width: 480, // Target width in pdf units?
                        windowWidth: 480 // Window width for rendering?
                    });
                }
            });
        }
            ...
            <div ref={protocolRef} style={{
                ...
                <div style={{ width: "100%" }}>
                    ...
                    <div id="printed-style">
                        {/* @ts-ignore */}
                        {props.summary.text && parse(marked(props.summary.text) as string)}
                        <hr /><br />
                        <Disclaimer />
                    </div>
                </div>
