 ...
 */
function Navbar({ topic, hamburgerOpen, setHamburgerOpen }: NavbarProps) {
    const isMobile = useMobile();
    const isMobileXs = useMobileXs();
    const isPortrait = usePortrait();
    const showIconinMeny = useMediaQuery({ query: '(min-width: 700px)' });
    const hamburgerAnimation = useRef<any>(null); // Lottie ref type is generic/any usually
    ...
    function handleOnNavigate(to: string) {
        navigate({
            hash: to
        });
    ...
    const navbarStyle: React.CSSProperties = {
        ...
        zIndex: "10" as unknown as number, // React types sometimes weird with zIndex strings, usually number
...
function NavItem({ name, isActive, show, onNavigate }: NavItemProps) {
    const { t } = useTranslation();
