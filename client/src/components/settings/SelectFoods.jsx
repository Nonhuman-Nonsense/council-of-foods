...
function SelectFoods({ topicTitle, onContinueForward }) {
  ...
  function continueForward() {
    if (atLeastTwoFoods() && selectedFoods.length <= maxFoods) {
      ...
      let participants = "";
      participatingFoods.forEach(function (id, index) {
      ...
      <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        >
          <div style={discriptionStyle}>
            ...
            <div>
              {!atLeastTwoFoods() ? <p>{t('selectbeings')}</p> : <><p>{t('wewilllisten')}:</p>
                <div>{selectedFoods.map((id) => <p style={{ margin: 0 }} key={food.type === 'panelist' ? food.id : foods.find(f => f.id === id).name}>{foods.find(f => f.id === id).name}</p>)}</div>
              </>}
            </div>
          </div>
          {infoToShow()}
