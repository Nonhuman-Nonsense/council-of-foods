...
interface TextOutputProps {
    currentAudioMessage: IAudioUpdatePayload | null | undefined;
    isPaused: boolean;

    setCurrentSnippetIndex: (index: number) => void;
    setSentencesLength: (length: number) => void;
    style?: CSSProperties; // Added optional style prop as it was passed in Output.jsx
}
    ...
    useEffect(() => {
        ...
        if (isPaused) {
            ...
            const animate = () => {
                ...
                // Note: startTimeRef.current is asserted non-null because we ensure it's set above
                const startTime = startTimeRef.current || now;
                const totalElapsedMS = accumulatedTimeRef.current + (now - startTime);
