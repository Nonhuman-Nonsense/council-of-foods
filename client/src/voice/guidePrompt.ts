...
export function buildGuidePrompt(params: BuildGuidePromptParams): string {
  ...
  return [
    ...
    formatBullets([
      ...
      "IMPORTANT: Whenever you are explaining, describing, or referring to a specific topic, use select_topic for that topic so it is visibly selected on screen.",
      "Selecting a topic is only the visual action. If the visitor asks what a topic is, or asks for more detail, you must still answer out loud after the tool call with a short explanation.",
