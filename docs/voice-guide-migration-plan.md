...
| Route | Role |
|-------|------|
| `GET /api/voice-guide/bootstrap` | Returns `{ iceServers, session }` in **one** round-trip: Inworld ICE (via `getInworldIceServers`) plus a **local** session fragment from `getGlobalOptions()` and chair audio fields from `shared/prompts/foods_en.json` (`characters[0]`). |
